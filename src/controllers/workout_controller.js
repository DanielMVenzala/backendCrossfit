...
const { all } = require("../v1/routes/workout_routes");
...
const getAllEquipment = (req, res) => {
  try {
    const allEquipment = workoutService.getAllEquipment();
    res.send({ status: "ok", data: allEquipment });
  } catch (error) {
    res.status(500).send({
      status: "FAILED",
      data: { error: error?.message || String(error) },
...
const getAllModes = (req, res) => {
  try {
    const allModes = workoutService.getAllModes();
    res.send({ status: "ok", data: allModes });
  } catch (error) {
    res.status(500).send({
      status: "FAILED",
      data: { error: error?.message || String(error) },
...
const getAllMembers = (req, res) => {
  ...
  try {
    const allMembers = workoutService.getAllMembers({ startsWith });
...
const getOneWorkout = (req, res) => {
  ...
  if (!id) {
    res.status(400).send({
      status: "FAILED",
      data: { error: "Tienes que introducir una ID" },
    });
    return;
  }
  try {
