...
const getAllWorkouts = (filterParams) => {
    try {
        ...
        throw { status: 500, message: error?.message || "Error al leer la base de datos" };
    }
};
    ...
    try {
        if (changes.name) {
            const isAlreadyAdded =
                DB.workouts.findIndex((workout) => workout.name === changes.name && workout.id !== id) > -1;
...
const createNewWorkout = (newWorkout) => {
    try {
        const workoutExist = DB.workouts.findIndex((workout) => workout.name === newWorkout.name) > -1;
...
const saveToDatabase = (DB) => {
    try {
        ...
        throw{status: 500, message: error?.message || "Error al guardar en la base de datos"}
    }
}

module.exports = {getAllWorkouts, getOneWorkout, createNewWorkout, saveToDatabase, deleteOneWorkout, updateOneWorkout}